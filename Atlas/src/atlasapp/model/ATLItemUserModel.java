...
public class ATLItemUserModel implements DBCallBackInterface{
	...
	public static ATLItemUserModelCallBackInterface atlItemUserModelListener;
	...
	protected ITEM_TYPE_PRIORITY_ORDER priority = ITEM_TYPE_PRIORITY_ORDER.OK;
...
public synchronized static int write(ItemUserProperties itemUserProperties) {
		...
		String sql = "insert into item_user (" 
				...
				+ "'" + DB.prep(itemUserProperties.priorityOrder.ordinal()-1) + "',"
...
//			if (webItem!=null && webItem.length>0)
				rowsDeleted += DB.delete("item_user", "web_item_id", new String[]{item}); 
...
public synchronized int upadteItemUserByItemUserId(HashMap<String,Object> itemUserUpdatedColumns, int itemUserId)
{
	...
	private ItemUserProperties fromHashMapToEventProperties(HashMap<String, String> itemUser) {
		...
		ITEM_TYPE_PRIORITY_ORDER priorityOrder =( (String) itemUser.get("priority_order")!=null)?
				(Integer.parseInt((String) itemUser.get("priority_order"))==0) ? ITEM_TYPE_PRIORITY_ORDER.OK:
					(Integer.parseInt((String) itemUser.get("priority_order"))==-1) ? ITEM_TYPE_PRIORITY_ORDER.DECLINED:
						 ITEM_TYPE_PRIORITY_ORDER.IDEAL:ITEM_TYPE_PRIORITY_ORDER.OK;
//						(Integer.parseInt((String) itemUser.get("priority_order"))==2) ? 	ITEM_TYPE_PRIORITY_ORDER.DECLINE:
//							(Integer.parseInt((String) itemUser.get("priority_order"))==-1) ? 	ITEM_TYPE_PRIORITY_ORDER.DECLINE:
//							ITEM_TYPE_PRIORITY_ORDER.IDEAL:ITEM_TYPE_PRIORITY_ORDER.IDEAL;
	...
	public static ArrayList<ItemUserProperties> getAllItemUserRecordsByItemId(String itemId)
	{
		...
		ArrayList<HashMap<String,String>> itemsH = DB.q("SELECT * FROM item_user WHERE item_id='" + itemId+"' order by display_order");
	...
	public static ArrayList<ItemUserProperties> getAllItemUserRecordsByWebItemId(String webItemId)
	{
		...
		{
			ArrayList<HashMap<String,String>> itemsH = DB.q("SELECT * FROM item_user WHERE web_item_id='" + webItemId+"' order by display_order");
	 ...
	 */
	public static HashMap<String,ArrayList<ItemUserProperties>> getAllItemUserRecordsByWebItemIds(
			ArrayList<String> webEventIds,HashMap<String, String> webEventIdByPrimry) {
		HashMap<String,ArrayList<ItemUserProperties>> allEventRecords = new HashMap<String,ArrayList<ItemUserProperties>>();

		if (webEventIds!=null && webEventIds.size()>0 && webEventIdByPrimry!=null && webEventIdByPrimry.size()>0)
		{
			...
			for (HashMap<String,String> rec:itemsH)
			{
				...
				if (rec!=null)
				{
					...
					primary = (webEventIdByPrimry.get(itemUser.itemUserProperties.webItemId));

					if (!allEventRecords.containsKey(primary))
						itemUserList = new ArrayList<ItemUserProperties>();
					else
						itemUserList = allEventRecords.get(primary);
					itemUserList.add(itemUser.itemUserProperties);
					allEventRecords.put(primary, itemUserList);
				}
			}
		}
	...
	public static ArrayList<ItemUserProperties> getAllItemUserRecordsByWebItemIds(
		...
		if (webEventIds!=null && webEventIds.size()>0)
		{
			...
			String primary = "";
			ArrayList<ItemUserProperties> itemUserList;
			ArrayList<String> webItemIdsFound = new ArrayList<String>();
			for (HashMap<String,String> rec:itemsH)
			{
				 itemUser = new ATLItemUserModel();
				 if (rec!=null)
				 {
					 ...
					 //						itemUserList = allEventRecords.get(primary);
					 //					itemUserList.add(itemUser.itemUserProperties);
					 if (itemUser.atlasId!=null && !itemUser.atlasId.equals("") &&
					   !webItemIdsFound.contains(itemUser.webItemUserId))
					 {
						 allEventRecords.add(itemUser.itemUserProperties);
						 webItemIdsFound.add(itemUser.webItemUserId);
					 }
				 }
			}
		}
	...
	public static ArrayList<ItemUserProperties> getAllInviteeItemUserRecordsByWebItemIds(
			ArrayList<String> webEventIds) {
		ArrayList<ItemUserProperties> allEventRecords = new ArrayList<ItemUserProperties>();
		...
		if (webEventIds!=null && webEventIds.size()>0)
		{
			...
			sql +=" and atlas_id!='"+ATLUser.getParseUserID()+"' order by display_order";
			ATLItemUserModel itemUser;
			ArrayList<HashMap<String,String>> itemsH = DB.q(sql);
			String primary = "";
			ArrayList<ItemUserProperties> itemUserList;
...
//					itemUserList.add(itemUser.itemUserProperties);
	...
	public static ArrayList<ATLItemUserModel> getAllEventItemRecords()
	{
		...
		ATLItemUserModel itemUser;
		ArrayList<HashMap<String,String>> itemsH = DB.q("SELECT * FROM item_user WHERE item_type='event' order by display_order");
		for (HashMap<String,String> rec:itemsH)
		{
			 itemUser = new ATLItemUserModel();
			if (rec!=null)
	 ...
	 */
	public synchronized static void updateAllItemUserRecordsLocally(ITEM_TYPE itemType,
			ArrayList<ItemUserProperties> allItemUserRec) {
		...
		if (allItemUserRec!=null && allItemUserRec.size()>0 && itemType!=null)
		{
			...
			Date date = new Date();
			String receivedDateTime ;// (itemUserProperties.receivedDateTime!=null)?itemUserProperties.receivedDateTime.toString():
			//date.toString();
			String statusDateTime ;
			for (ItemUserProperties itemUserProperties:allItemUserRec)
			{
				receivedDateTime = (itemUserProperties.receivedDateTime!=null)?itemUserProperties.receivedDateTime.toString():
					date.toString();
				statusDateTime = (itemUserProperties.statusDateTime!=null)?itemUserProperties.statusDateTime.toString():
					date.toString();
				itemUserProperties = ItemUserProperties.deNull(itemUserProperties);
				...
				if (allWebItemUserIds.contains(itemUserProperties.webItemUserId))
				{
					String sql = "update  item_user SET " 
							...
							+ "status_datetime ='"+ DB.prep(statusDateTime) + "'," 
							+ "was_received ='"+ DB.prep(itemUserProperties.wasReceived) + "'," 
							+ "received_datetime ='"+ DB.prep(receivedDateTime) + "'," 
							+ "priority_order ='"+ DB.prep(itemUserProperties.priorityOrder.ordinal()-1) + "',"
...
//			if (atlItemUserModelListener!=null)
//				atlItemUserModelListener.writeItemUserOnLocalDB(true);
			
		}
	}
	private static ArrayList<ItemUserProperties> allItemUserRec;
	public  static void UpdateAllItemUserRecordsLocallyInBackground(ITEM_TYPE itemType,
			ArrayList<ItemUserProperties> allItemUserRecU)
	{
		if (allItemUserRecU!=null && allItemUserRecU.size()>0)
		{
			allItemUserRec = allItemUserRecU;
//			UpdateAllItemUserRecordsLocallyInBackground updateAllItemUserRecordsLocallyInBackground = new
//					UpdateAllItemUserRecordsLocallyInBackground();
//			updateAllItemUserRecordsLocallyInBackground.execute();
//		}
//	}
//	private static ArrayList<ItemUserProperties> allItemUserRec;
//	public static void  UpdateAllItemUserRecordsLocallyInBackground(){
//		@Override
...
//			ArrayList<ItemUserProperties> allItemUserRec) {
		...
		if (allItemUserRec!=null && allItemUserRec.size()>0 && ITEM_TYPE.EVENT!=null)
		{
			
			String sql;
			
			ArrayList<String> eventsIdsRecordsOnLocal = null;
				 sql = "SELECT web_item_user_id FROM item_user WHERE item_type='"+ITEM_TYPE.EVENT.getItemName() +"'";
				eventsIdsRecordsOnLocal = getAllRecordsBySQL(sql);
			ArrayList<String> allWebItemUserIds = eventsIdsRecordsOnLocal;
			allWebItemUserIds = (allWebItemUserIds!=null)? allWebItemUserIds: new ArrayList<String>();
			for (ItemUserProperties itemUserProperties:allItemUserRec)
			{
				itemUserProperties = ItemUserProperties.deNull(itemUserProperties);
			...
			if (atlItemUserModelListener!=null)
				atlItemUserModelListener.updatedItemUserOnLocalDB(true);
//			CurrentSessionEvents currentSessionEvents = CurrentSessionEvents.getCurrentSessionEventsSingleton();
//			currentSessionEvents.setItemUserUpdated(true);
...
//	}
	 ...
	 * 			of the item_user
	 */
	public static ArrayList<String> getAllCurrentWebItemUserIdsOnLocal(ITEM_TYPE itemType) {
			ArrayList<String> eventsIdsRecordsOnLocal = null;
			if (itemType!=null)
			{
				String sql = "SELECT web_item_user_id FROM item_user WHERE item_type='"+itemType.getItemName() +"' order by display_order";
				eventsIdsRecordsOnLocal = getAllRecordsBySQL(sql);
			}
		return eventsIdsRecordsOnLocal;
	}

	public static ArrayList<String> getAllCurrentWebItemIdsOnLocal(ITEM_TYPE itemType) {
		ArrayList<String> eventsIdsRecordsOnLocal = null;
		if (itemType!=null)
		{
			String sql = "SELECT web_item_id FROM item_user WHERE item_type='"+itemType.getItemName() +"' order by display_order";
			eventsIdsRecordsOnLocal = getAllRecordsBySQL(sql);
		}
	return eventsIdsRecordsOnLocal;  
}
	...
	public static ArrayList<String> getUserMoveWebItemIds(ITEM_TYPE itemType) {
		ArrayList<String> userMoves = null;
		if (itemType!=null)
		{
			String sql = "SELECT web_item_id FROM item_user WHERE item_type='"+itemType.getItemName() +"' AND atlas_id='"+ATLUser.getParseUserID()+"' AND status=0 order by display_order";
			userMoves = getAllRecordsBySQL(sql);
		}
	return userMoves;
	}
