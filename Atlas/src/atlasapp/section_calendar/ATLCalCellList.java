...
public class ATLCalCellList {
	...
	public boolean isListEditable = true;
	
	private ArrayList<ATLCalendarModel> calListModel;
	private ArrayList<String> calendarInActiveNameArray;
	
	public ATLCalCellList(ArrayList<ATLCalendarModel> calListModel,ArrayList<String> calendarInActiveNameArray) {
		...
		this.calendarInActiveNameArray = calendarInActiveNameArray;
		this.calListModel = calListModel;
	}

	// METHODS
	public boolean load(Context ctx,ArrayList<EventProperties> events) {
		mContext = ctx;
		clear();
		...
		if (this.calListSimulate) {
			...
			for (int ktr = 0; ktr < 240; ktr++) {
				...
				if ((x % 10 == 0) | (x == 100) | (x == 101) | (x == 102)
					...
					newCell.isEditable = this.isListEditable;
			...
			int inactiveSize = calendarInActiveNameArray.size();
			int calendarListSize = calListModel.size();
			ArrayList<Integer> ignoreList = new ArrayList<Integer>();
			for(int i = 0; i < inactiveSize; i++){
				String calName = calendarInActiveNameArray.get(i);
				for(int y = 0; y < calendarListSize; y ++){
					if(calName.trim().toUpperCase().equals(calListModel.get(y).name.trim().toUpperCase())){
						ignoreList.add(calListModel.get(y).id);
						break;
					}
				}
			}
			...
			for (int i = 0; i < 24; i++) {
				...
				ArrayList<EventProperties> eventsPropertiesInhour = null;
				if (events!=null)
				  eventsPropertiesInhour = ATLCalendarManager
						.eventsPropertiesInHourOfDay(events, i, this.calListDate);
				// check: if TRUE=> must add a blank item.
				
//				}
				if (eventsInhour.size() > 0 || eventsPropertiesInhour!=null && eventsPropertiesInhour.size()>0)
				{
					if (eventsPropertiesInhour!=null && eventsPropertiesInhour.size()>0)
						this.addNewPendingCell(eventsPropertiesInhour);
					if (eventsInhour.size() > 0)
						for (ATLEventModel event : eventsInhour) {
							this.createNewCellWithEvent(event,ctx,null);
						}
//					for (EventProperties event:eventsPropertiesInhour)
//					{
	...
	public void createBlankCellWithHour(int hour, int minute) {
		...
		for (int index = 0; index < size; index++) {
			ATLCalCellData cellDataOld = (ATLCalCellData) calListArray
					.get(index);
			cellDataOld.isEditable = this.isListEditable;
	...
	public void createNewCellWithEvent(ATLEventModel event,Context context,ArrayList<EventProperties> events) {
		// TODO Auto-generated method stub
		ATLCalCellData newCell = new ATLCalCellData();
		newCell.isEditable = this.isListEditable;
		...
		//////
		ATLAlarmCellModelProperties model = ATLAlarmCellModel.getAlarmById(event.event_id);
		if (model!=null)
		{
			newCell.calCellEventGroupID = model.calendarId;
			newCell.calCellEventIdentifier = model.calendarId;
			newCell.calCellPreferDateTime = model.itemDatetime;
			
			newCell.inviterAtlasId = model.atlasId;
...
//			newCell.calCellALT3Datetime = ATLEventGroupDatabaseAdapter.getDateFromIDString(eventGroup.calCellAlt3EventIdentifier);
			newCell.eventResponseType_CellData = model.respondStatus;
		}
...
//		}
		if (!newCell.eventResponseType_CellData.equals(EventResponseType.Deleted)
				&&  !newCell.eventResponseType_CellData.equals(EventResponseType.Decline))
		{	
			this.addCell(newCell);
			this.calEventsListArray.add(newCell);
		}
		if (model!=null && model.calendarId!=null && !model.calendarId.equals("")&& !model.calendarId.equals("-1"))
		{
			long deletedRows = Long.valueOf(model.calendarId);
			if (newCell.eventResponseType_CellData.equals(EventResponseType.Deleted)
					||  newCell.eventResponseType_CellData.equals(EventResponseType.Decline)
					||  newCell.eventResponseType_CellData.equals(EventResponseType.NoAttendees)
					)
				deletedRows = ATLCalendarCellController.deleteAlarmCellFromCalendarByID(deletedRows,context);
		}
		if (events!=null)
			addNewPendingCell(events);
	} 

	public void addNewPendingCell(ArrayList<EventProperties> events)
	{
		if (events!=null && events.size()>0)
		{
			...
			{
				newCell = new ATLCalCellData();
				newCell.isEditable = this.isListEditable;
				newCell.calCellStartDate = event.startDateTime;
				Calendar cal = new GregorianCalendar(TimeZone.getDefault());
				cal.setTime(newCell.calCellStartDate);
				newCell.calCellPreferDateTime = newCell.calCellStartDate;
				newCell.calCellTitle = event.title;
				newCell.calCellDate = cal.getTime();
				newCell.calCellCalendarColor = Color.YELLOW;
				newCell.calCellHour = cal.get(Calendar.HOUR_OF_DAY);
				newCell.calCellMinute = cal.get(Calendar.MINUTE);
				newCell.calCellEndDate = event.endDateTime;
				newCell.calCellDurationMinutes = (int) (((event.endDateTime.getTime() - event.startDateTime.getTime())/1000)/60);
				newCell.calCellGUID = this.uuid();
				newCell.calCellLocation = event.location;
				newCell.calCellEventIdentifier = "";
				newCell.isBlank = false;
				newCell.calCellEndDate = event.endDateTime;
				newCell.calCellNotes = event.notes;
				newCell.calendarId = 0;
				newCell.inviterAtlasId = event.atlasId;
				newCell.eventId = 0; // Fixed move and delete events
				
				newCell.calCellPreferDateTime = event.startDateTime;
				
				newCell.eventResponseType_CellData = EventResponseType.Pending;
