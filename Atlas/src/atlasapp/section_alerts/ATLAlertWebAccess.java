...
import atlasapp.common.SendEmailActivity;
...
public class ATLAlertWebAccess {
	...
	public ATLAlertWebAccess() {
		...
		userId = ATLUser.getParseUserID();
		userEmail = ATLUser.getEmail();
		userDisplayname = ATLUser.getUserNameDisplay();
		username = ATLUser.getEmail();
	}
		...
		newAcceptSend.saveInBackground(new SaveCallback() {
			public void done(ParseException e) {
				...
				if (e == null) {
					...
					String message =  ATLUser.getUserNameDisplay()+" has confirmed "+eventTitle+" at "+timeChosen;
	...
	private void asyncEventInvitedSendWithData(Object item) {
		...
		sendString = data.calCellTitle + "," + data.calCellLocation + ","
				...
				+ data.attendee.get(0).displayName();
