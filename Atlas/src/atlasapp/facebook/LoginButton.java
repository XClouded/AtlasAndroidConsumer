...
//            this.setGravity(Gravity.CENTER);
    ...
    public void setReadPermissions(List<String> permissions) {
        if (AuthorizationType.PUBLISH.equals(authorizationType)) {
            throw new UnsupportedOperationException(
                    "Cannot call setReadPermissions after setPublishPermissions has been called.");
        }
        if (validatePermissions(permissions, AuthorizationType.READ)) {
    ...
    public void setPublishPermissions(List<String> permissions) {
        if (AuthorizationType.READ.equals(authorizationType)) {
            throw new UnsupportedOperationException(
                    "Cannot call setPublishPermissions after setReadPermissions has been called.");
        }
        if (validatePermissions(permissions, AuthorizationType.PUBLISH)) {
            this.permissions = permissions;
            authorizationType = AuthorizationType.PUBLISH;
        }
    }

    private boolean validatePermissions(List<String> permissions, AuthorizationType authType) {
        if (AuthorizationType.PUBLISH.equals(authType)) {
            if (FacebookUtilities.isNullOrEmpty(permissions)) {
                throw new IllegalArgumentException("Permissions for publish actions cannot be null or empty.");
            }
        }
        Session openSession = sessionTracker.getOpenSession();
        if (openSession != null) {
            if (!FacebookUtilities.isSubset(permissions, openSession.getPermissions())) {
...
//        }
    ...
    private boolean initializeActiveSessionWithCachedToken(Context context) {
        ...
        String applicationId = getMetadataApplicationId(context);
    ...
    private  boolean reLoginFB = false;
    public   void reLoginFacebookATLUser(boolean reLogin)
    {
    	reLoginFB = reLogin;
    }
