...
public class ATLFriendLocalTable {
	...
	public static void updateAllFriendOnDBInBackground(boolean register)
	{
		firstRegister = register;
		//UpdateOnDB longOperation = new UpdateOnDB();
		UpdateOnDB2 longOperation = new UpdateOnDB2();
		longOperation.execute("write","all");
	}
	public static void readAllFriendsOnDBInBackground()
	{
	...
	public static class  UpdateOnDB extends AsyncTask<String, Integer, String> {
		...
		protected String doInBackground(String... params) {
			...
			if (backgroundOperation.equals("write"))
			{
				...
				if (ATLUser.getfbID() != null && !ATLUser.getfbID().equals("")
	...
	public static void retrieveNewFriendsByAtlasIds(ArrayList<String> friendsIds)
	{
		if (friendsIds!=null && friendsIds.size()>0)
		{
			newFriendsIds = friendsIds;
			RetrieveNewFriendsByAtlasIds retrieveNewFriendsByAtlasIds = new RetrieveNewFriendsByAtlasIds();
			retrieveNewFriendsByAtlasIds.execute();
		}
	}
	private static ArrayList<String> newFriendsIds = null;
	public static class  RetrieveNewFriendsByAtlasIds extends AsyncTask<String, Integer, String> {
...
//				return atlasIdA;
				ArrayList<String> currentAtlasIdsOnLocal = atlasIdA;
				ArrayList<String> newAtTLFriends=null;
				if (currentAtlasIdsOnLocal!=null && currentAtlasIdsOnLocal.size()>0)
				{
					newAtTLFriends = new ArrayList<String>();
					for (String atlasId:newFriendsIds)
					{
						if (!currentAtlasIdsOnLocal.contains(atlasId))
							newAtTLFriends.add(atlasId);
					}
					
					if (newAtTLFriends!=null && newAtTLFriends.size()>0)
					{
						AtlasServerConnect.setNewAtlasFriendsFromAtlasIdsListInBackground(newAtTLFriends);
					}
					
				}else
				{
					AtlasServerConnect.setNewAtlasFriendsFromAtlasIdsListInBackground(newFriendsIds);
				}
	...
	public static class  UpdateOnDB2 extends AsyncTask<String, Integer, String> {

		@Override
		protected String doInBackground(String... params) {
			boolean newRegister = firstRegister;
			firstRegister = false;
			ctx = UtilitiesProject.ctx;
			String[] operation = params;
			String backgroundOperation = operation[0];
			...
			HashMap<String,String> emailsByContactIndex = new HashMap<String,String>();
			friendsToUpdate = (friendsToUpdate!=null)? friendsToUpdate : "";
			if (backgroundOperation.equals("write"))
			{
				if (friendsToUpdate.equals("address") || friendsToUpdate.equals("all"))
				{
					...
					{  
						...
						while (cur.moveToNext()) 
						{
							...
							////////
							///Checking  if the conatct has a name....
							String whereName = ContactsContract.Data.MIMETYPE + " = ? AND " + ContactsContract.CommonDataKinds.StructuredName.CONTACT_ID + " = ?";
							whereName = (whereName==null)? "":whereName;  
							String[] whereNameParams = new String[] { ContactsContract.CommonDataKinds.StructuredName.CONTENT_ITEM_TYPE, id };
							Cursor nameCur = cr.query(ContactsContract.Data.CONTENT_URI, null, whereName, whereNameParams, ContactsContract.CommonDataKinds.StructuredName.GIVEN_NAME);
							...
							{   
								...
								while (emailCur!=null && emailCur.moveToNext() && !isUserEmail )
								{ 
									// This would allow you get several email addresses
									// if the email addresses were stored in an array
									email = (emailCur.getString(
											emailCur.getColumnIndex(ContactsContract.CommonDataKinds.Email.DATA)));
									isUserEmail = ATLUser.getEmailAddresses().contains(email);
									if(!currentABContactsEmailAdd.contains(email) && !isUserEmail){
										emails.add(email);
										emailsByContactIndex.put(email, Integer.toString(contactIndex));
								...
								conatctModel.setFriendEmailAddresses(emails);
								emailCur.close();
								...
								if (!isUserEmail && ("1").equals(cur.getString(cur.getColumnIndex(ContactsContract.Contacts.HAS_PHONE_NUMBER)))) 
								{
									Cursor pCur = cr.query(
											ContactsContract.CommonDataKinds.Phone.CONTENT_URI,
											null,
								...
								if (!isUserEmail && phoneNum.length > 0 && phoneNum[0]!=null && !phoneNum[0].equals(""))
									conatctModel.setPhoneNumber(phoneNum[0]); 

								final Bitmap photo = retrieveContactPhoto(id);
								if (!isUserEmail && photo!=null)
									conatctModel.setImage(photo);  

								if (!isUserEmail && conatctModel.getEmailAddress()!=null && !conatctModel.getEmailAddress().equals("") 
										&&conatctModel.getFirstname()!=null  && !conatctModel.getFirstname().equals(""))
									contactLocalProperties.put(Integer.toString(contactIndex), conatctModel);
						...
						atlDBCommon.setAllCurrentSessionContactsEmailsOnMobile(emailsByContactIndex);

						atlDBCommon.setCurrentSessionUsersEmails(currentABContactsEmailAdd);

						atlDBCommon.setContactModelFromMobile(contactLocalProperties);

						parseConnect.setAllUsersContactsEmailsOnParse(currentABContactsEmailAdd,newRegister);
							...
							&&((friendsToUpdate.equals("facebook")||friendsToUpdate.equals("all"))))
					{
						...
						parseConnect.scanCurrentNewFacebookFriendsOnParse(facebookFriends);
...
//				   
		   ...
		   File imgFile;
		   if (!imageObjectId.equals(ATLUser.getParseUserID()))
		   {  imageDir =
