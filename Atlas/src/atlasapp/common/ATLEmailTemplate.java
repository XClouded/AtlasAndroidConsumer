...
public class ATLEmailTemplate 
{
	 ...
	 * @return
	 */
	public static String emailTemplateForEvent(ATLContactModel invitee, ArrayList<EventProperties> event,
			ArrayList<ItemUserProperties> itemUser,
			EmailTemplateType emailTemplateType)
	{
		Log.v("ATLAS EMAIL TEMPLATE", "IN OUTBOUND SEND");

		if (event==null || event.size()==0 || itemUser==null || itemUser.size()==0 || invitee==null || emailTemplateType==null)
			return null;
		
		ATLContactModel myUser = ATLContactModel.getContactByEmailAddress(invitee.getEmailAddress());//invitee;//
		  Log.v("OUT BOUND EMAIL", " USER ATLAS ID"+itemUser.get(0).atlasId);
		  Log.v("OUT BOUND EMAIL", " USER ATLAS ID"+myUser.getAtlasId());

		String primaryWebItemUserId = getPrimaryWebItemUserId(event,itemUser);
		  Log.v("OUT BOUND EMAIL", " primaryWebItemUserId "+primaryWebItemUserId+" EVENT "+event.get(0).title);

		
		String webSite = UtilitiesProject.getATLWebSite(ATL_ENVIRONMENT.QA);
		if (primaryWebItemUserId==null || primaryWebItemUserId.equals("")||
				webSite==null || webSite.equals("") || myUser==null)
			return null;
//		  Log.v("OUT BOUND EMAIL", " webSite "+webSite);
		...
		String bodyTable=bodyTableTemplateWithTitle(event.get(0).title, event.get(0).message);
		  ...
		  Log.v("OUT BOUND EMAIL", " bodyTable "+bodyTable);
		...
		headerImage =ATLConstants.kImageUrl+headerImage;// @"http://getatlas.com/emails/img/"
	
		String bodyback =   commonTemplateWithHeaderImageUrl(headerImage,
				header, body, 
				appStoreUrl ,webUrl, 
				webUrl);
//		Log.v("ATLAS EMAIL TEMPLATE", "primaryWebItemUserId exsitsts");
		Log.v("ATLAS EMAIL TEMPLATE", "primaryWebItemUserId exsitsts"+bodyback);
		return bodyback;
	}
			...
			ArrayList<ItemUserProperties> itemUser)
	{
		...
		if (event!=null && event.size()>0 && itemUser!=null && itemUser.size()>0)
		{
			...
			for (EventProperties eventAlt:event)
			  ...
			  Log.v("OUT BOUND EMAIL", " EVENT PRIMARY"+primary);  

			if (primary!=null && !primary.equals(""))
			{
				for (ItemUserProperties itemUserRec:itemUser)
				{
					  Log.v("OUT BOUND EMAIL", "  EVENT ITEM PRIMARY "+itemUserRec.webItemId);
				...
				String forEmailAddressId, String emailAddressId)
	{
		String webSite = UtilitiesProject.getATLWebSite(ATL_ENVIRONMENT.QA);
		 String webUrl = webSite+"/verify/"+ emailAddressId;
		  ...
		  //http://getatlas.com/emails/img/top-blue-bg4.png
		  String headerImage ="top-blue-bg4.png";// (ATLUser.getImageUrl()!=null &&!ATLUser.getImageUrl().equals(""))?ATLUser.getImageUrl():"top-blue-bg4.png";
		  ...
		  headerImage =ATLConstants.kImageUrl+headerImage;
	...
	{
		String webUrl = UtilitiesProject.getATLWebSite(ATL_ENVIRONMENT.QA);
