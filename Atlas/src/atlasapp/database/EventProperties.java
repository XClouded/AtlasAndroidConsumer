...
public class EventProperties {
	...
	public ATLContactModel eventInviterModel;
	/**
	...
	public EventProperties(String setWebEventId,
	        ...
	        ,boolean isPrimary,EVENT_TYPE setEventType,String setPhoneNumber,String setMessage)
	{
		...
		this.phoneNumber = (setPhoneNumber!=null)? setPhoneNumber:phoneNumber;
		...
		this.message = (setMessage!=null)? setMessage:message;
		
		setInviterModel();
	}
	private ATLContactModel setUserAsContactModel() {
		ATLContactModel userModel = new ATLContactModel();
		
		userModel.setAtlasId(ATLUser.getParseUserID());
		userModel.setEmailAddress(ATLUser.getEmail());
		userModel.setFacebookId(ATLUser.getfbID());
		userModel.setFirstname(ATLUser.getUserFirstName());
		userModel.setLastname(ATLUser.getUserLastName());
	...
	public EventProperties(ParseObject eventParseObject)
	{
		if (eventParseObject!=null && eventParseObject.getClassName().equals("event"))
		{
			...
			this.message =  (eventParseObject.getString("message")!=null)? eventParseObject.getString("message"):this.message;
			this.phoneNumber = (eventParseObject.getString("phone_number")!=null)? eventParseObject.getString("phone_number"):this.phoneNumber;
			...
			EVENT_STATUS statusInt = (eventParseObject.getNumber("status")!=null)?
						((Integer)eventParseObject.getNumber("status")==1)? EVENT_STATUS.THE_ONE:
							((Integer)eventParseObject.getNumber("status")==2)?EVENT_STATUS.NOT_THE_ONE:
								((Integer)eventParseObject.getNumber("status")==3)?EVENT_STATUS.COMPLITED:
			...
			this.isPrimary = primaryWebEventId.equals(webEventId) || primaryWebEventId.equals("");
			
//			if (atlasId!=null && !atlasId.equals(ATLUser.getParseUserID()))
//			{
				setInviterModel();
//			}
//			else
//				eventInviterModel = setUserAsContactModel();
	...
	public void setInviterModel()
	{
		if (atlasId!=null && !atlasId.equals(ATLUser.getParseUserID()))
		{
			eventInviterModel = ATLContactModel.getContactByAtlasId(atlasId);
		}
		else
			eventInviterModel = setUserAsContactModel();
	...
	public static EventProperties deNull(EventProperties eventProperties)
	{
		...
		//	eventProperties.duration =0;
			...
			//the inviter id
			eventProperties.message = (eventProperties.message!=null)?eventProperties.message:"";

			eventProperties.phoneNumber = (eventProperties.phoneNumber!=null)?eventProperties.phoneNumber:"";
