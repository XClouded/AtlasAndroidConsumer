...
//	}
	...
	private synchronized static final int delete(String tableName,String where,
		    String []  values, SQLiteDatabase db2) {
		if (db2 == null) {
			openDb();
			db2 = db;
		}
		valuesToDelete = values;
		String column = where+"=?";
		
		
		DeleteRecordsInTheBackground deleteRecordsInTheBackground = new DeleteRecordsInTheBackground();
		deleteRecordsInTheBackground.execute(new String[]{tableName,column});
		
		
		return 	0;//	db.delete(tableName,column, values);
	...
	static String []  valuesToDelete;
	public static class  DeleteRecordsInTheBackground extends AsyncTask<String, Integer, String> {

		@Override
		protected String doInBackground(String... params) {
			
			String[] operation = params;
			String tableName = operation[0];
			String column = operation[1];
		
			db.delete(tableName,column, valuesToDelete);
			return null;
		}
