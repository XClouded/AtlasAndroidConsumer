 ...
 * 
 */
public class ATLAlertEventRequest_2 extends Activity implements
		OnClickListener, ATLAlertWebAccessCallBackInterface,
		ATLAlertEventRequestCellDelegate, EventControllerCallBackInterface {
	...
	public EventProperties eventPropertiesPre = null;
	public EventProperties eventPropertiesAlt1 = null;
	public EventProperties eventPropertiesAlt2 = null;
	ATLAlert alert = null;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// Get data from singleton objects
		{
			...
			if (isSimulateMode) {
				...
				// currentCellData =
				// AlertRequestEventSingleton.getInstance().getAlertCellData();
				// Log.v("currentCellData.getEventPrimaryKey",
				// currentCellData.getEventPrimaryKey());
...
//						.getEventPropertiesAlt2();
				alert = AlertRequestEventSingleton.getInstance().getAlert();
...
//						eventPropertiesPre.webEventId);
	...
	private void setListeners() {
		...
		decline.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				eventController.respondToEventInvite(
						eventPropertiesPre.webEventId, null);
	...
	private void changeBgColorOfCell(Date aDate) {
		...
		for (int i = 0; i < listSize; i++) {
			...
			if (aDateCalString.equals(aCellCalString)) {
				...
				cellData.calCellTitle = eventPropertiesPre.title;
				cellData.eventResponseType_CellData = EventResponseType.eventResponseType_InvitesPending;
			...
			for (int i = 0; i < size; i++) {
				if (atlIndexs.get(i) == index) {
					...
					case 1:// alt2 date
						...
						eventController.respondToEventInvite(
								eventPropertiesPre.webEventId,
								eventPropertiesAlt1.webEventId);
						break;
					case 2:// alt3 date
						...
						eventController.respondToEventInvite(
								eventPropertiesPre.webEventId,
								eventPropertiesAlt2.webEventId);
						break;
					default:
						break;
					}
	...
	public void respondToEventInviteCallBack(boolean success) {
		...
		if (success) {
			...
			switch (chosenItem) {
			case EVENT_REQUEST_DECLINE: {
//				AlertCellData cellData = AlertRequestEventSingleton
//						.getInstance().getAlertCellData();
...
//				alertModel.save();
	...
	public void bookEventCallBack(boolean success) {
		...
		if (success) {
			...
			// "Respons to event successfully updated",
			// Toast.LENGTH_LONG).show();
			Log.v("bookEventCallBack to event successfully updated",
					"bookEventCallBack to event successfully updated");
...
//				alertModel.save();
			...
			finish();
			overridePendingTransition(R.anim.stand_by, R.anim.out_to_bottom);
			Log.v("bookEventCallBack to event failed to updated",
					"bookEventCallBack to event failed to updated");
		}
	}
